...
import AllViewsPanelOption2 from './AllViewsPanelOption2';
import { ViewTypeIcon, TeamFavRowIcon } from '../components/viewIcons';
...
// Option 2's own starting point: same catalogue/roles as the live app, but
// "Pending" and "Closed" start out pinned in Favourites rather than
// auto-pinned to the sidebar — a ready-made example of the "drag from
// Favourites into your sidebar" interaction as soon as the page loads.
const buildInitialViewsByInbox = () => {
  const cloned = JSON.parse(JSON.stringify(INITIAL_VIEWS_BY_INBOX));
  Object.values(cloned).forEach((inbox) => {
    inbox.favouritesOrder = inbox.favouritesOrder || {};
    ['Admin', 'Agent'].forEach((role) => {
      const sidebar = inbox.sidebarOrder[role] || [];
      const toDemote = ['pending', 'closed'].filter((id) => sidebar.includes(id));
      inbox.sidebarOrder[role] = sidebar.filter((id) => !toDemote.includes(id));
      inbox.favouritesOrder[role] = [...(inbox.favouritesOrder[role] || []), ...toDemote];
    });
  });
  return cloned;
};
...
const MainSidebarPanelOption2 = ({ activeFilter, onFilterChange, activeRole }) => {
  ...
  const [viewsByInbox, setViewsByInbox] = React.useState(buildInitialViewsByInbox);
  ...
  const renderNestedItems = (inboxName) => {
    const { views, sidebarOrder: sidebarOrderByRole, teamFavouriteIds } = viewsByInbox[inboxName];
    const sidebarOrder = sidebarOrderByRole[activeRole] || [];
    const viewsById = {};
    views.forEach((v) => { viewsById[v.id] = v; });

    // What shows in the sidebar is exactly `sidebarOrder` (max
    // MAX_FAVOURITES) — becoming a favourite or Team Favourite no longer
    // auto-pins a view here; it only lands here once the user drags it from
    // the All Views "Favourites" section into "In your sidebar".
...
export default MainSidebarPanelOption2;
